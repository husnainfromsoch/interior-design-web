    ...
    <section className="py-24">
      <div className="mx-auto grid max-w-[1320px] grid-cols-1 items-center gap-14 px-6 lg:grid-cols-[0.85fr_1.15fr] lg:px-8">
        <div className="reveal-left order-2 lg:order-1">
          ...
          <ul className="mt-8 grid max-w-md grid-cols-1 gap-8 border-t border-bv-line/60 pt-8 sm:grid-cols-3 sm:gap-6">
            {trustPoints.map(({ key, Icon }, i) => (
              <li
                key={key}
                className={`flex flex-col items-start gap-3 ${
                  i > 0 ? "sm:border-l sm:border-bv-line/50 sm:pl-6" : ""
                }`}
              >
                <span className="flex h-10 w-10 items-center justify-center rounded-full bg-bv-accent/10">
                  <Icon className="h-5 w-5 text-bv-accent" strokeWidth={1.75} aria-hidden />
                </span>
                <span className="text-[14px] font-semibold leading-tight">
                  {t(`points.${key}.title`)}
                </span>
                <span className="text-[13px] leading-relaxed text-bv-muted">
                  {t(`points.${key}.body`)}
                </span>
              </li>
            ))}
