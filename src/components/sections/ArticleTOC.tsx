...
export default function ArticleTOC({
  ...
  return (
    <nav aria-label={label} className="hidden lg:sticky lg:top-[120px] lg:block">
      <span className="block font-[var(--font-bv-body)] text-[13px] font-semibold uppercase tracking-[0.08em] text-bv-ink">
        {label}
      </span>
...
export function ArticleTOCMobile({
  ...
  return (
    <details className="mb-8 border-y border-bv-line py-4 lg:hidden">
      <summary className="cursor-pointer font-[var(--font-bv-body)] text-[13px] font-semibold uppercase tracking-[0.08em] text-bv-ink">
        {label}
      </summary>
      <ul className="mt-4 space-y-3">
        {headings.map((h) => (
