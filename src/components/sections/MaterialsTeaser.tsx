...
const swatches = ["#C9C2B6", "#8B6F52", "#98583F", "#332E2B"];
    ...
    <section className="bg-bv-surface py-24">
      <div className="mx-auto grid max-w-[1320px] grid-cols-1 items-stretch gap-14 px-6 lg:grid-cols-[1.2fr_1fr] lg:px-8">
        ...
        <div className="reveal-right">
          ...
          <ul className="mt-8 grid max-w-md grid-cols-1 gap-6 border-t border-bv-line/60 pt-7 sm:grid-cols-3">
            {trustPillars.map(({ key, Icon }) => (
              <li key={key} className="flex flex-col items-start gap-2">
                <Icon className="h-6 w-6 text-bv-accent" strokeWidth={1.5} aria-hidden />
                <span className="text-[14px] font-semibold leading-tight">
                  {t(`trust.${key}.title`)}
                </span>
                <span className="text-[13px] leading-snug text-bv-muted">
                  {t(`trust.${key}.body`)}
                </span>
              </li>
            ))}
