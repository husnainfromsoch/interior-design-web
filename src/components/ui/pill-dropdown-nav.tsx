  ...
  return (
    <div className="relative md:hidden">
      <button
        ...
        className="flex h-11 w-11 items-center justify-center rounded-full border-2 border-bv-ink bg-bv-background text-bv-ink"
      >
        <motion.span
        ...
        >
          <motion.span
            ...
            className="h-0.5 w-full origin-center rounded-full bg-bv-ink"
          />
          <motion.span
            variants={{ closed: { opacity: 1 }, open: { opacity: 0 } }}
            className="h-0.5 w-full rounded-full bg-bv-ink"
          />
          <motion.span
            variants={{
              closed: { rotate: 0, y: 0 },
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            {onSwitchLocale && (
              <div className="mt-1 flex items-center justify-center gap-1.5 border-t border-bv-line/60 px-2 pb-1 pt-3">
                <button
                  type="button"
                  onClick={() => {
                    onSwitchLocale("en");
